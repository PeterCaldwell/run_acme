...
set run_name   = linoz_test        # !!!!!! CHANGE BEFORE ARCHIVING to run_acme_example  !!!!!!!
set job_name   = $run_name              # !!!!!! CHANGE BEFORE ARCHIVING to $run_name  !!!!!!!
set compset    = FC5CLBMG2LINMAM4RESUSMOMBC # !!!!!! CHANGE BEFORE ARCHIVING to A_B1850  !!!!!!!
...
set records_per_atm_output_file  = 48   # !!!!!! CHANGE BEFORE ARCHIVING to 40   !!!!!!!

### SOURCE CODE OPTIONS (2)
set fetch_code = false                  # !!!!! CHANGE BEFORE ARCHIVING to true !!!!!!!
set acme_tag   = susburrows/atm/atm_integration_stage4  # !!!!! CHANGE BEFORE ARCHIVING to master   !!!!!!!
set tag_name   = atm_integration_stage4 # !!!!!  CHANGE BEFORE ARCHIVING to run_acme_tag_example !!!!!!!
...
set seconds_before_delete_run_dir    = 10   # !!!!!!! CHANGE BEFORE ARCHIVING to -1 !!!!!!

### !!!! OPTIONS BELOW HERE NORMALLY SHOULDN'T BE CHANGED !!!!
### ----------------------------------------------------------
### PROCESSOR CONFIGURATION (6)
set processor_config = S   # To run ACME pre-alpha on Titan, set this to use the 'custom' configuration. # !!!!!!! CHANGE BEFORE ARCHIVING to M (or to 'custom' for pre-alpha version) !!!!!!
...
#./xmlchange -file env_build.xml -id CAM_CONFIG_OPTS -val "-phys cam5 -chem trop_mam4_resus_mom -rain_evap_to_coarse_aero "
#./xmlchange -file env_build.xml -id CAM_CONFIG_OPTS -val "-phys cam5 -chem trop_mam4_mom -rain_evap_to_coarse_aero "
#./xmlchange -file env_build.xml -id CAM_CONFIG_OPTS -val "-phys cam5 -chem trop_mam4_mom "
...
cat <<EOF >> user_nl_cam
 ...
 soil_erod_file		= '${inputdata_dir}/atm/cam/dst/dst_1.9x2.5_c090203.nc'
 xs_long_file		= '${inputdata_dir}/atm/waccm/phot/temp_prs_GT200nm_JPL10_c130206.nc'
 rsf_file		= '${inputdata_dir}/atm/waccm/phot/RSF_GT200nm_v3.0_c080416.nc'
 exo_coldens_file	= '${inputdata_dir}/atm/cam/chem/trop_mozart/phot/exo_coldens.nc'
EOF
...
if ( `lowercase $debug_queue` == 'true' ) then
  ...
  if ( $machine == cab ) then
    ...
    sed -i /"#SBATCH${cime_space}--job-name"/a"#SBATCH  --time=01:00:00"     ${case_name}.run
  else if ( $machine == titan || $machine == eos  ) then
